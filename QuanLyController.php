...
class QuanLyController extends Controller
{
    ...
    public function xuLyCapNhat(Request $request, $id)
    {
        ...
        foreach($files as $ha){
            $pic = new HinhAnhQuanLy;
            $pic->quan_ly_id=$quanly->id;
            $path=$ha->store('images/quanly');
            $pic->url=$path;
    ...
    public function chitiet($id){
        $HinhQL=HinhAnhQuanLy::all();
        $quanly = quanly::find($id);
        if (empty( $quanly)) {
            return redirect()->route('quanly.danh-sach')->with('thong_bao', 'NHÂN VIÊN KHÔNG TỒN TẠI');
        }
        return view('quanly.chi-tiet', compact('quanly', 'HinhAnh'));
    }
}
